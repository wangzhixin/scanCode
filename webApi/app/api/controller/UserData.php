  ...
  public function exportUserData()
  {
    ...
    if ($ids) {
      ...
      foreach ($getOrderList as &$each) {
        $this->getUserData($each);
        $list[] = array($each['name'], $each['type'] == '1' ? '正常' : '异常', $each['user_type'], $each['id_type'], $each['id_number'], $each['phone_number'], $each['province_value'], $each['city_value'], $each['district_value'], $each['address'], $each['hospital_value'], $each['department_value'], $each['invalid_time']);
      }
      $filename = '填报信息' . time();
